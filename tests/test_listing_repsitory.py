...
def test_all_returns_all_listings(db_connection):
    ...
    assert result == [
                    Listing(1, 'first-test-listing', 45, 1),
                    Listing(2, 'second-test-listing', 70, 1),
                    Listing(3, 'third-test-listing', 35, 2),
                    Listing(4, 'fourth-test-listing', 50, 3),
                    Listing(5, 'fifth-test-listing', 100, 2),
                    Listing(6, 'sixth-test-listing', 85, 1)]
...
def test_find_by_user_returns_given_users_listings(db_connection):
    ...
    assert result == [Listing(4, 'fourth-test-listing', 50, 3)]

# create() adds a new listed to the database 
def test_create_adds_new_listing(db_connection):
    db_connection.seed('seeds/makers_bnb.sql')
    repository = ListingRepository(db_connection)
    repository.create(None, 'seventh-test-listing', 38, 3)
    result = repository.all()
    assert result == [
                    Listing(1, 'first-test-listing', 45, 1),
                    Listing(2, 'second-test-listing', 70, 1),
                    ...
                    Listing(5, 'fifth-test-listing', 100, 2),
                    Listing(6, 'sixth-test-listing', 85, 1),
                    Listing(7, 'seventh-test-listing', 38, 3)]
