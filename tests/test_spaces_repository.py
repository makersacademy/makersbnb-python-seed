...
def test_list_all_space(db_connection):
    ...
    assert repository.all_listings() == [Space(1, 'Apartment 1', 'Description 1', 100, datetime.date(2024, 1, 1), datetime.date(2024, 4, 1)),
                                Space(2, 'Apartment 2', 'Description 2', 200, datetime.date(2024, 1, 1), datetime.date(2024, 4, 1)) 
                            ]
...
def test_add_new_space_listing(db_connection):
    db_connection.seed("seeds/spaces.sql")
    new_listing = Space(3, 'Apartment 3', 'Description 3', 100, datetime.date(2024, 1, 1), datetime.date(2024, 2, 3))
    repository =  SpacesRepository(db_connection)
    ...
    assert repository.all_listings() == [
        Space(1, 'Apartment 1', 'Description 1', 100, datetime.date(2024, 1, 1), datetime.date(2024, 4, 1)),
        Space(2, 'Apartment 2', 'Description 2', 200, datetime.date(2024, 1, 1), datetime.date(2024, 4, 1)),
        Space(3, 'Apartment 3', 'Description 3', 100, datetime.date(2024, 1, 1), datetime.date(2024, 2, 3))
                            ]