...
def test_find_property_using_user_id__has_properties(db_connection):
    db_connection.seed("seeds/makers_bnb_database.sql")
    repository = PropertyRepository(db_connection)
    assert repository.find_property_user_id(1) == [
        Property(1,'Hackers Hideaway', 'A mystery place full of bugs', 50.00, 1),
        Property(2,'Ma house', 'Its a very nice', 25.5, 1)
    ]

def test_find_property_using_user_id_no_property(db_connection):
    db_connection.seed("seeds/makers_bnb_database.sql")
    repository = PropertyRepository(db_connection)
    assert repository.find_property_user_id(3) == []