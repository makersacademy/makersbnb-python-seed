...
def test_class_instantiates_correctly():
    listing = Listing(1, 'first listing', 35, 1)
    assert listing.id == 1
...
def test_instances_equality_with_identical_values():
    listing1 = Listing(1, 'first listing', 35, 1)
    listing2 = Listing(1, 'first listing', 35, 1)
    assert listing1 == listing2

# Instance returns nicely formatted string when printed
def test_string_formatting():
    listing1 = Listing(1, 'first listing', 35, 1)
    assert str(listing1) == "Listing(1, first listing, 35, 1)"